...
%% indpensim code
function [X,unom_Fs,unom_PAA,yest] = indpensim(f_input, Xd, x0, h, T, solv, p, Ctrl_flags,Recipe_Fs,Recipe_Fs_sp,Recipe_PAA,Recipe_PAA_sp,contr_param)
...
X = createBatch(h,T);
cp = [50/h:contr_param.step/h:230/h];
% User control inputs; 
...
%    waitbar(k / N);
    ...
    if any(k==cp)% Add MPC controller
        [Recipe_Fs_sp,Recipe_PAA_sp]=fast_controller(X,contr_param,k*h,Recipe_Fs_sp,Recipe_PAA_sp);
    end
    ...
    % gets MVs
    [u X] = f_input(X, Xd, k, h, T,Ctrl_flags,Recipe_Fs,Recipe_Fs_sp,Recipe_PAA,Recipe_PAA_sp);
...
X.OUR.t(k) =  t_sol(end); 
% Calculating the CER
X.CER.y(k) = (44*X.Fg.y(k)/22.4)*((0.65*X.CO2outgas.y(k)/100)*(0.7902/(1-O2_in-X.CO2outgas.y(k)/100)-X.CO2outgas.y(k)/100));
X.CER.t(k) = t_sol(end);
