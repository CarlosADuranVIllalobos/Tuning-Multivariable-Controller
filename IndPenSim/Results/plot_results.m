...
violin(boxes,'facecolor',[b;o;y;g;p;lightb;r;b2]);
hold on; 
xL = get(gca,'XLim');
...
y_vcall = [];
for i=1:1:length(rep_names)
    y_vcall = [y_vcall; Fast_MPC.(char(rep_names(i))).output];
end

figure(2)
boxes=[y_nom,y_vcall, y_lin,y_exp, y_de, y_bci, y_bci2] ;
...
savefig('pen_comp')
 ...
 plot(y_mpc1(1:80),'Color',y,'LineStyle',':','Marker','o','linewidth',1.5,'MarkerSize',8);
 plot(y_mpc4(1:80),'Color',p,'LineStyle',':','Marker','x','linewidth',1.5,'MarkerSize',8);
 plot(y_mpc6(1:80),'Color',r,'LineStyle',':','Marker','d','linewidth',1.5,'MarkerSize',8);
 ...
 set(findall(gcf,'-property','FontSize'),'FontSize',24)
 legend('Nominal','M=1','M=1e-3','M=1e-5','Set-point', 'FontSize',20,'Location','best');
 set(gcf, 'Color', 'w');
 savefig('progression_constant')
