...
batch_rep=3;  % Number of replicates
...
for m=1:1:batch_rep
    ...
    for i=1:1:batch_ini
        ...
        Recipe_Fs_sp(cp:end)=Recipe_Fs_sp(cp:end)+idinput(timeinterval-cp+1,'rbs',[0 .05],[-u_disturb*max(Rec_Fs) u_disturb*(Rec_Fs)])';
        Recipe_PAA_sp(cp:end)=Recipe_PAA_sp(cp:end)+idinput(timeinterval-cp+1,'rbs',[0 .05],[-u_disturb*max(Rec_PAA) u_disturb*max(Rec_PAA)])';
