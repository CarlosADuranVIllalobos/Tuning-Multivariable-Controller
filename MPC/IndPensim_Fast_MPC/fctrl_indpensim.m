...
function [u X] = fctrl_indpensim(X, Xd, k, h, T, Ctrl_flags,Recipe_Fs,Recipe_Fs_sp,Recipe_PAA,Recipe_water_PAA)
...
Recipe_Fg_sp = [30  42  55   60  75   60   60 ];
...
%% SBC - F_PAA
%Recipe_PAA =       [25 200  1000 1750 ]; % Sequential batch control 
%Recipe_water_PAA = [5  0    10    0  ];
for SQ = 1:1:length(Recipe_water_PAA) % Fg Sequence number 
% User input aeration rates
if k<=Recipe_PAA(SQ)
    Fpaa =  Recipe_water_PAA(SQ);
    break
else 
    Fpaa =  Recipe_water_PAA(end);
end 
end
% SBC - NH3_shots 
Xd.NH3_shots.y(k)= 0;
...
%
% Please reference  "The Development of an Industrial Scale Batch
